...
import { ChevronLeft, ChevronRight, Star, Quote, AlertCircle, MessageSquare } from 'lucide-react';
  ...
  return (
    <div style={{ background: COSMOS }} className="min-h-screen">
      ...
      >
        ...
        {/* Glowing quote marks */}
        <motion.div className="absolute top-24 left-12 text-9xl font-serif select-none pointer-events-none"
          style={{ color: 'rgba(245,158,11,0.05)' }}
          animate={{ opacity: [0.04, 0.09, 0.04] }} transition={{ duration: 4, repeat: Infinity }}>
          "
        </motion.div>
        <motion.div className="absolute bottom-8 right-16 text-9xl font-serif select-none pointer-events-none"
          style={{ color: 'rgba(252,211,77,0.04)' }}
          animate={{ opacity: [0.03, 0.07, 0.03] }} transition={{ duration: 5, repeat: Infinity, delay: 2 }}>
          "
        </motion.div>
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 text-center w-full">
          ...
          >
            Voices of <span style={{ background: 'linear-gradient(90deg,#F59E0B,#FCD34D)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>the Stars</span>
          </motion.h1>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
            className="text-lg text-white/60 mb-2">
            Real stories from real people whose lives have been transformed by Vedic wisdom
          </motion.p>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
            ...
            <span className="ml-3 text-white/70 text-sm">4.9 / 5 from 50,000+ clients</span>
          </motion.div>
        </div>
      </div>

      {loading ? (
        <div className="py-24 flex items-center justify-center" style={{ background: COSMOS }}>
          <Loading />
        </div>
      ) : error ? (
            ...
            <section className="py-20" style={{ background: '#08060f' }}>
              <div className="max-w-4xl mx-auto px-4">
                <div className="text-center mb-10">
                  <p className="text-xs uppercase tracking-widest mb-2" style={{ color: 'rgba(245,158,11,0.7)' }}>Featured Stories</p>
                  <h2 className="text-3xl font-serif text-white">What Our Clients Say</h2>
                </div>
                ...
                <div className="relative rounded-3xl overflow-hidden"
                  ...
                  <div className="relative overflow-hidden" style={{ minHeight: 340 }}>
                    <AnimatePresence initial={false} custom={direction}>
                      ...
                      >
                        {/* Big quote mark */}
                        <Quote className="w-8 h-8 mb-4" style={{ color: 'rgba(245,158,11,0.3)' }} />
                        ...
                        <img src={featured[currentIndex].avatar} alt={featured[currentIndex].name}
                          className="w-16 h-16 rounded-full object-cover mb-3"
                          style={{ border: '2px solid rgba(245,158,11,0.5)', boxShadow: '0 0 20px rgba(245,158,11,0.2)' }} />

                        <div className="flex gap-1 mb-2">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <Star key={i} className={`w-4 h-4 ${i < featured[currentIndex].rating ? 'fill-current' : ''}`}
                              style={{ color: i < featured[currentIndex].rating ? '#F59E0B' : 'rgba(255,255,255,0.2)' }} />
                          ))}
                        </div>
                        <h3 className="font-semibold text-white">{featured[currentIndex].name}</h3>
                        <p className="text-sm text-white/40 mb-2">{featured[currentIndex].location}</p>
            ...
            <div className="max-w-7xl mx-auto px-4">
              <div className="text-center mb-14">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                  <p className="text-xs uppercase tracking-widest mb-3" style={{ color: 'rgba(245,158,11,0.7)' }}>50,000+ Satisfied Clients</p>
                  <h2 className="text-4xl font-serif text-white mb-3">All Client <span style={{ background: 'linear-gradient(90deg,#F59E0B,#FCD34D)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Reviews</span></h2>
                  <p className="text-white/40">Every story is a journey transformed by the light of Jyotish</p>
                </motion.div>
              </div>
              ...
              {testimonials.length > 0 ? (
                ...
                <div className="text-center py-16">
                  <p className="text-white/30 text-lg">No testimonials available yet.</p>
                </div>
              )}
            </div>
          </section>
            ...
            <div className="max-w-2xl mx-auto px-4 text-center">
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                <div className="rounded-3xl p-12"
                  ...
                  <MessageSquare className="w-12 h-12 text-saffron mx-auto mb-4"
                    style={{ filter: 'drop-shadow(0 0 16px rgba(245,158,11,0.5))' }} />
                  <h2 className="text-3xl font-serif text-white mb-4">Your Story Matters</h2>
                  <p className="text-white/50 mb-8">
                    Have you consulted with us? We'd love to hear how Vedic wisdom transformed your life. Share your experience and inspire others.
                  </p>
                  <a href={`https://wa.me/919876543210`} target="_blank" rel="noopener noreferrer"
                    ...
                    style={{ background: 'linear-gradient(90deg,#22c55e,#16a34a)', color: 'white', boxShadow: '0 0 30px rgba(34,197,94,0.3)' }}>
